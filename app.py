...
Features:
  ...
  • Live FastAPI backend connectivity (auto-detects port)
  • Forensic PDF generation + RSA integrity verification
  • Download PDF button connected to backend
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
# Backend port can be overridden via env var SAFESTAY_API_PORT
API_PORT = int(os.environ.get("SAFESTAY_API_PORT", "8000"))
...
def api_download(path: str, timeout: int = 15) -> bytes | None:
    """Download binary content (e.g. PDF) from the backend."""
    try:
        r = requests.get(f"{API_BASE}{path}", timeout=timeout)
        if r.ok:
            return r.content
...
with st.sidebar:
    ...
    if api_status is True:
        st.success(f"API Online ✓  (:{API_PORT})")
    ...
    st.divider()
    st.markdown(
        f'<div style=\'font-family:"Space Mono",monospace;font-size:.65rem;color:#6B7A99\'>'
        f'Backend: {API_BASE}</div>',
        unsafe_allow_html=True
    )
...
with ev_col1:
    ...
    if st.button("📄 Generate Report PDF", key="gen_report"):
        with st.spinner("Building forensic PDF…"):
            ...
            if meta:
                ...
                # Cache the PDF bytes for download button
                if st.session_state.api_online:
                    pdf_bytes = api_download("/api/report/download")
                    if pdf_bytes:
    ...
    if st.session_state.report_meta:
        ...
        # Download PDF button — works via backend stream or local file
        pdf_bytes = getattr(st.session_state, "report_pdf_bytes", None)
        if pdf_bytes:
            st.download_button(
                label="⬇ Download PDF Report",
                data=pdf_bytes,
                file_name="SafeStay_Forensic_Report.pdf",
...
if st.session_state.api_online:
    ...
    if stats:
        st.markdown("### 📊 Backend Stats")
        s1, s2, s3, s4 = st.columns(4)
        s1.metric("Total Sessions",  stats.get("sessions_total", 0))
        s2.metric("Last Level",       stats.get("last_level", "—"))
        s3.metric("Network Cameras", stats.get("network_cameras", 0))
        s4.metric("Report Ready",    "✓" if stats.get("report_ready") else "✗")
        st.divider()
